...
public sealed class CSVManager
{
    ...
    // CSV 파일 읽기: 각 행을 string[]로 반환
    public List<string[]> Read(string filePath, Encoding? encoding = null)
    {
    ...
    // CSV 파일 쓰기: 각 행을 string[]로 받아 파일에 저장
    public void Write(string filePath, IEnumerable<string[]> rows, Encoding? encoding = null)
    {
        encoding ??= Encoding.UTF8;
