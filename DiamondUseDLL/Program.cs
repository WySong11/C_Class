    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            /*            Trace.Listeners.Add(new ConsoleTraceListener()); // 콘솔에도 출력
                        Trace.Listeners.Add(new TextWriterTraceListener("log.txt")); // 파일에도 출력
                        Trace.Listeners.Add(new DefaultTraceListener()); // 기본 리스너 추가*/

            // AutoFlush를 true로 설정하면 로그가 즉시 파일에 기록됩니다.
            // 기본적으로 Trace는 버퍼링되어, 프로그램 종료 시에만 파일에 기록됩니다.

            //Trace.AutoFlush = true;
            //Trace.WriteLine($"{DateTime.Now:yyyy-MM-dd_HH-mm-ss} : 파일로 저장되는 로그입니다.");
